...
// ================== Konfigurasi WiFi AP ==================
const char* AP_SSID = "KelompokKeren";   // Nama WiFi
const char* AP_PASS = "Kelompok1";       // Password min 8 karakter
...
void handleRoot() {
  String html = F(
    ...
    ".grid{display:grid;grid-template-columns:100px 100px 100px;justify-content:center;align-items:center;margin-top:20px;}"
    ".grid div{margin:5px;}"
    ".stop{background:#dc3545;}"
    ".led{background:#28a745;}"
    ".off{background:#6c757d;}"
    "</style></head><body>"
    "<h1>Kontrol Robot Sumo</h1>"

    "<h2>Kontrol Gerakan</h2>"
    "<div class='grid'>"
    ...
    "<h2>Kontrol Lampu</h2>"
    "<p><a class='btn led' href='/led/on'>LED ON</a>"
    " | <a class='btn off' href='/led/off'>LED OFF</a></p>"
...
void setup() {
  ...
  digitalWrite(LED_BUILTIN, HIGH); // LED mati default (LOW = nyala di ESP8266)
  ...
  // LED
  server.on("/led/on", [](){ digitalWrite(LED_BUILTIN, LOW); server.send(200,"text/plain","LED ON"); });
  server.on("/led/off", [](){ digitalWrite(LED_BUILTIN, HIGH); server.send(200,"text/plain","LED OFF"); });
